...
export const ChannelGrid: React.FC<ChannelGridProps> = ({
  ...
  // Channels retain natural arrangement order
  ...
  return (
    <div id="tv-channel-section" className="space-y-3 pb-6">
      {/* Top Controls Bar: Clean Category Tabs + View Density Toggle */}
      <div className="flex items-center justify-between gap-3 bg-[#0A0A0A] p-2 sm:p-2.5 rounded-xl border border-white/5 flex-wrap md:flex-nowrap">
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`h-8 sm:h-9 px-3 sm:px-3.5 rounded-lg text-xs sm:text-[13px] font-semibold transition-all duration-150 flex items-center gap-1.5 whitespace-nowrap shrink-0 select-none ${
                  isSelected
                    ? 'bg-[#F8F9FA] text-neutral-900 shadow-md font-bold border border-neutral-200 ring-2 ring-orange-500/40'
                    : 'bg-[#141414] text-[#E0E0E0]/80 hover:text-white hover:bg-white/10 border border-white/5'
                } ${isCategoryFocused ? 'ring-2 ring-orange-500 scale-105' : ''}`}
              >
                {cat === 'All' && <Tv className="w-3.5 h-3.5 text-orange-400" />}
                {cat === 'News' && <Radio className="w-3.5 h-3.5 text-red-400" />}
                {cat === 'Cable TV' && <Tv2 className="w-3.5 h-3.5 text-cyan-400" />}
                {cat === 'Music' && <Music className="w-3.5 h-3.5 text-purple-400" />}
                ...
                {cat === 'Entertainment' && <Film className="w-3.5 h-3.5 text-pink-400" />}
                {cat === 'Movies' && <Film className="w-3.5 h-3.5 text-blue-400" />}
        ...
        {/* View Density Toggle & Count */}
        <div className="flex items-center gap-2.5 shrink-0">
          <span className="text-[11px] font-mono font-bold text-white/50 bg-white/5 px-2 py-1 rounded border border-white/5 hidden sm:inline">
            {displayChannels.length} CHANNELS
          </span>

          {/* View Density / Card Size Switcher */}
          <div className="flex items-center bg-[#121212] border border-white/10 rounded-lg p-0.5">
            <button
              ...
              title="Fit to screen (Ultra-compact)"
            >
      ...
      {displayChannels.length === 0 ? (
          ...
          {displayChannels.map((channel, index) => {
            ...
            return (
              <div
                ...
                className={`group relative flex items-center gap-2.5 bg-[#121212] rounded-xl px-2.5 py-2 min-h-[52px] transition-all duration-150 cursor-pointer text-left overflow-hidden select-none ${
                  isFocused
              ...
              >
                ...
                {/* Channel Logo */}
                <div className="w-9 h-9 rounded-lg bg-[#080808] p-0.5 border border-white/10 shrink-0 flex items-center justify-center overflow-hidden group-hover:border-orange-500/40 transition-colors shadow-inner">
                  <img
                    src={channel.logo}
                    alt={channel.name}
          ...
          {displayChannels.map((channel, index) => {
            ...
            return (
              <div
                ...
                id={`channel-card-${channel.id}`}
                onClick={() => onSelectChannel(channel)}
                className={`group relative flex flex-col justify-between bg-[#121212] rounded-lg p-2.5 min-h-[96px] transition-all duration-150 cursor-pointer text-left overflow-hidden ${
                  isFocused
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-3.5 sm:gap-4">
          {displayChannels.map((channel, index) => {
            const isFocused = focusedZone === 'channels' && focusedChannelIndex === index;
            ...
            return (
              <div
                ...
                id={`channel-card-${channel.id}`}
                onClick={() => onSelectChannel(channel)}
                className={`group relative flex flex-col justify-between bg-[#121212] rounded-xl p-3.5 min-h-[175px] transition-all duration-200 cursor-pointer text-left overflow-hidden ${
                  isFocused
